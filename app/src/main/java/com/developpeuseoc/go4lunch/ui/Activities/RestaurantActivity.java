...
import com.developpeuseoc.go4lunch.ui.Adapters.RestaurantWorkmatesAdapter;
...
public class RestaurantActivity extends AppCompatActivity implements Serializable {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Place.PlaceDetailsResult placeDetailsResult = null;
        if (bundle != null) {
            placeDetailsResult = (Place.PlaceDetailsResult) bundle.getSerializable("placeDetailsResult");
        }
    ...
    //For update UI
    private void updateUI(Place.PlaceDetailsResult placeDetailsResult, RequestManager glide) {
        this.mGlide = glide;
    ...
    private void restaurantRating(Place.PlaceDetailsResult placeDetailsResult) {
    ...
    public void selectedRestaurant() {
        ...
        if (placeDetailsResult != null) {
            UserRepository.updatePlaceId(Objects.requireNonNull(FirebaseUtils.getCurrentUser()).getUid(), placeDetailsResult.getPlaceId(), getCurrentTime());
            okFloatingButton.setImageDrawable(getResources().getDrawable(R.drawable.ic_check_selected));
    ...
    public void removeRestaurant() {
        UserRepository.deletePlaceId(Objects.requireNonNull(Objects.requireNonNull(FirebaseUtils.getCurrentUser()).getUid()));
        okFloatingButton.setImageDrawable(getResources().getDrawable(R.drawable.ic_check_unselected));
                ...
                public void onSuccess(DocumentSnapshot documentSnapshot) {
                    ...
                    if (user != null) {
                        if (!user.getLike().isEmpty() && user.getLike().contains(placeRestaurantId)) {
                            UserRepository.deleteLike(FirebaseUtils.getCurrentUser().getUid(), placeRestaurantId);
                            starButton.setBackgroundResource(R.color.quantum_grey);
                        } else {
                            UserRepository.updateLike(FirebaseUtils.getCurrentUser().getUid(), placeRestaurantId);
